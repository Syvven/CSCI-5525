...
regrets_per_round = np.zeros((4, 2, rounds_per))
for rep in range(repetitions):
    ...
    for round in range(rounds_per):
        ...
        for i,agent_pair in enumerate(agents):
            ...
            cum_regret[i][1] += ustars[1] - agent_pair[1].means[lever_gauss]

            # update the totals
            regrets_per_round[i][0][round] += cum_regret[i][0]
            regrets_per_round[i][1][round] += cum_regret[i][1]
...
ax1.plot(regrets_per_round[0][0], label="Random Agent")
ax1.plot(regrets_per_round[1][0], label="Epsilon Greedy")
ax1.plot(regrets_per_round[2][0], label="UCB")
ax1.plot(regrets_per_round[3][0], label="Thompson Sampling")
...
ax2.plot(regrets_per_round[0][1], label="Random Agent")
ax2.plot(regrets_per_round[1][1], label="Epsilon Greedy")
ax2.plot(regrets_per_round[2][1], label="UCB")
ax2.plot(regrets_per_round[3][1], label="Thompson Sampling")
